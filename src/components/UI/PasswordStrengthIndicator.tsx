...
const PasswordStrengthIndicator: React.FC<PasswordStrengthIndicatorProps> = ({ password, show }) => {
  ...
  return (
    <div className="mt-2 p-3 bg-white/20 rounded-lg border border-white/50">
      <div className="mb-2">
        <div className="flex items-center justify-between text-sm mb-1">
          <span className="text-white">Fortaleza de contraseña</span>
          <span className={`font-medium ${strength === 3 ? 'text-green-300' : 'text-white'}`}>
      ...
      <ul className="space-y-1">
        {requirements.map((req, index) => (
          <li key={index} className="flex items-center space-x-2 text-sm">
            ...
            <span className={req.test ? 'text-green-200' : 'text-white/80'}>
              {req.text}
            </span>
          </li>
        ))}
