...
interface PartnerAprobacionesListProps {
  onStatsUpdate: () => void;
}

const PartnerAprobacionesList: React.FC<PartnerAprobacionesListProps> = ({ onStatsUpdate }) => {
  const { admin } = useAdmin();
  const [solicitudes, setSolicitudes] = useState<PartnerSolicitud[]>([]);
  ...
  const fetchSolicitudes = async () => {
    try {
      ...
      console.error('Error fetching partner solicitudes:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async (solicitudId: string) => {
    ...
    try {
      ...
      console.error('Error approving partner solicitud:', error);
    } finally {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      console.error('Error rejecting partner solicitud:', error);
    } finally {
      setProcessingId(null);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/15 backdrop-blur-lg rounded-2xl p-6 shadow-2xl border border-cyan-200/30">
        <h3 className="text-xl font-bold text-white mb-6 flex items-center">
          ...
          Solicitudes de Socios Pendientes ({pendientes.length})
        </h3>
        
        {pendientes.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-white/70">No hay solicitudes de socios pendientes</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h4 className="text-white font-semibold">
                        {solicitud.partners.nombre}
                      </h4>
                      <p className="text-white/70 text-sm">{solicitud.partners.email}</p>
                      <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                        solicitud.partners.tipo === 'operador_partner'
                          ? 'bg-yellow-500/20 text-yellow-300 border border-yellow-500/50'
                          : 'bg-blue-500/20 text-blue-300 border border-blue-500/50'
                      }`}>
                        {solicitud.partners.tipo === 'operador_partner' ? 'SOCIO + OPERADOR' : 'SOCIO'}
                      </span>
                    </div>
                  </div>
      ...
      {/* Solicitudes Procesadas de Partners */}
      <div className="bg-white/15 backdrop-blur-lg rounded-2xl p-6 shadow-2xl border border-cyan-200/30">
        <h3 className="text-xl font-bold text-white mb-6 flex items-center">
          <CheckCircle className="w-6 h-6 mr-3 text-green-300" />
          Solicitudes de Socios Procesadas ({procesadas.length})
        </h3>
        
        {procesadas.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-white/70">No hay solicitudes de socios procesadas</p>
          </div>
        ) : (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-white/60 text-sm">{solicitud.partners.email}</p>
                    </div>
                  </div>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md">
            <h3 className="text-xl font-bold text-gray-900 mb-4">Rechazar Solicitud de Socio</h3>
...
export default PartnerAprobacionesList;