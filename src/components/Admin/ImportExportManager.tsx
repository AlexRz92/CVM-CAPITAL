...
const ImportExportManager: React.FC<ImportExportManagerProps> = ({ onUpdate }) => {
  ...
  const fetchDefaultModule = async () => {
    try {
      // Buscar el módulo predeterminado "C.V.M Capital"
      const { data: defaultModule, error } = await supabase
      ...
      if (defaultModule) {
        setDefaultModuleId(defaultModule.id);
        console.log('Módulo predeterminado encontrado:', defaultModule.id);
      }
    } catch (error) {
      console.error('Error fetching default module:', error);
    }
  ...
  const handleImportInversores = async () => {
    if (!selectedFile) {
      showError(
        'Archivo Requerido',
        'Por favor selecciona un archivo CSV antes de continuar.'
      );
      return;
    }
    ...
    if (!defaultModuleId) {
      showError(
        ...
        'No se encontró el módulo predeterminado "C.V.M Capital". Contacta al administrador.'
      );
      return;
    }
    ...
    try {
      ...
      for (const inv of inversores) {
        try {
          ...
          // Asignar automáticamente al módulo predeterminado
      ...
      showError(
        ...
        'Error al importar los datos: ' + (error as Error).message
      );
    } finally {
      setImporting(false);
    }
  ...
  const handleExportInversores = async () => {
    ...
    try {
      ...
      showError(
        ...
        'Error al exportar los datos: ' + (error as Error).message
      );
    } finally {
      setExporting(false);
    }
  ...
  const handleExportPartners = async () => {
    ...
    try {
      ...
      showError(
        'Error de Exportación',
        'Error al exportar los datos de partners: ' + (error as Error).message
      );
    } finally {
      setExporting(false);
    }
            ...
            <div className="mb-4 p-4 bg-blue-50 rounded-lg">
              ...
              <div className="mt-3 text-sm text-blue-700">
                ...
                <ul className="list-disc list-inside mt-1">
                  ...
                  <li>Los inversores se asignan automáticamente al módulo "C.V.M Capital"</li>
                  <li>Las transacciones se registran en el módulo predeterminado</li>
                </ul>
              </div>
            </div>
