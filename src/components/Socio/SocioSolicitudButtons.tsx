...
const SocioSolicitudButtons: React.FC = () => {
  ...
  useEffect(() => {
    if (partner) {
      setSaldoActual(partner.inversion_inicial);
    }
  }, [partner]);
  ...
  const validateWithdrawAmount = async (amount: number): Promise<boolean> => {
    try {
      const { data, error } = await supabase.rpc('validar_retiro_partner', {
  ...
  const handleWithdrawSubmit = async () => {
    ...
    try {
      ...
      setWithdrawAmount('');
      setSuccessMessage('Estaremos validando su retiro, por favor espere un mínimo de 48H. De haber pasado 48H y no se refleje comuníquese por Correo: pnf.alexisruiz@gmail.com o Telegram: @TheAlexRz92');
      setShowSuccessModal(true);
    } catch (error) {
