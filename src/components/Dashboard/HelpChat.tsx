...
const HelpChat: React.FC<HelpChatProps> = ({ userId, userType }) => {
  ...
  const fetchCurrentTicket = async () => {
    ...
    try {
      const { data, error } = await supabase.rpc('obtener_ticket_usuario', {
        p_usuario_id: userId,
        p_tipo_usuario: userType
      });
  ...
  const handleSubmitTicket = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.rpc('crear_ticket', {
        ...
        p_mensaje: formData.mensaje.trim()
      });
      ...
      if (data.success) {
        ...
        if (data.existing_ticket) {
          setCurrentTicket(data.existing_ticket);
          setHasTicket(true);
        }
      }
    } catch (error) {
      console.error('Error creating ticket:', error);
      alert('Error al crear el ticket. Inténtalo más tarde.');
    } finally {
      setLoading(false);
    }
  };
