...
interface TemporalTransactionsTableProps {
  transactions: TemporalTransaction[];
}

const TemporalTransactionsTable: React.FC<TemporalTransactionsTableProps> = ({ transactions }) => {
  ...
  return (
    <div className="bg-white/15 backdrop-blur-lg rounded-2xl p-6 shadow-2xl border border-cyan-200/30">
      <h3 className="text-xl font-bold text-white mb-6 text-center">
        Historial de Transacciones Temporales
      </h3>
      
      {transactions.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-white/70">No hay transacciones temporales registradas</p>
        </div>
      ) : (
          ...
          {ganancias.length > 0 && (
            <div>
              <h4 className="text-lg font-semibold text-yellow-300 mb-4 flex items-center">
                ...
                Ganancias Temporales ({ganancias.length})
              </h4>
                    ...
                    <tbody>
                      {ganancias.map((transaction) => (
                        <tr key={transaction.id} className="border-b border-yellow-500/20 hover:bg-yellow-500/10 transition-colors">
                          ...
                          <td className="py-3 px-3 text-yellow-100 text-sm">
                            {transaction.descripcion || 'Ganancia temporal'}
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          {otrasTransacciones.length > 0 && (
            <div>
              <h4 className="text-lg font-semibold text-white mb-4">
                Otras Transacciones Temporales ({otrasTransacciones.length})
              </h4>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
          ...
          {ganancias.length === 0 && otrasTransacciones.length > 0 && (
            <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
              <p className="text-blue-200 text-sm text-center">
                <strong>Sin ganancias temporales registradas:</strong> Las ganancias aparecerán aquí cuando sean procesadas por el administrador
              </p>
            </div>
          )}
        </div>
...
export default TemporalTransactionsTable;