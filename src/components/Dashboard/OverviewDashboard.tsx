...
const OverviewDashboard: React.FC = () => {
    ...
    try {
      ...
      const accesos = await Promise.all(
        modulos.map(async (modulo) => {
          const tieneAcceso = await verificarAcceso(modulo.id, user.id, 'inversor');
      ...
      console.log('Módulos con acceso encontrados:', modulosConAcceso);
  ...
  const cargarResumenModulos = async (modulosIds: string[]) => {
    ...
    try {
      ...
      console.error('Error cargando resumen de módulos:', error);
      setModulosResumen([]);
    }
  };
  ...
  const getActivityIcon = (tipo: string) => {
    switch (tipo.toLowerCase()) {
      ...
      case 'retiro':
        return <ArrowDownRight className="w-4 h-4 text-white" />;
      case 'ganancia':
                  ...
                  <div key={modulo.id} className="bg-white/10 rounded-lg p-4 border border-white/20 hover:bg-white/15 transition-colors">
                    ...
                    {modulo.ultima_ganancia > 0 && modulo.fecha_ultima_ganancia && (
                      <div className="mt-3 pt-3 border-t border-white/20">
                        <p className="text-white">Última ganancia:</p>
                        <div className="flex items-center justify-between">
                          <p className="text-yellow-200 font-semibold">{formatCurrency(modulo.ultima_ganancia)}</p>
                          <p className="text-yellow-200 text-xs">{formatDate(modulo.fecha_ultima_ganancia)}</p>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <div className="bg-white/15 backdrop-blur-lg rounded-2xl p-6 shadow-2xl border border-cyan-200/30">
              ...
              <div className="space-y-4">
                  ...
                  <div key={modulo.id} className="bg-white/10 rounded-lg p-4">
                    ...
                    <div className="w-full bg-white/20 rounded-full h-2">
                      <div 
                        ...
                        style={{ 
                          width: `${resumenGeneral.saldo_total_modulos > 0 ? (modulo.saldo_actual / (resumenGeneral.saldo_total_modulos - resumenGeneral.saldo_principal)) * 100 : 0}%` 
                        }}
                      ></div>
                    </div>
                ...
                {resumenGeneral.saldo_principal > 0 && (
                  <div className="bg-white/10 rounded-lg p-4 border-2 border-blue-400/50">
                    <div className="flex items-center justify-between mb-2">
