/*
  # Corregir constraint de configuración y cálculo de ganancias

  1. Arreglar constraint único de configuración_sistema
  2. Implementar cálculo correcto de ganancias para partners
  3. Agregar configuración para porcentaje de inversores
  4. Funciones actualizadas para distribución correcta
*/

-- Primero eliminar constraint existente si hay conflicto
DO $$ 
BEGIN
...
-- Función para obtener distribución de partners con cálculo correcto
...
BEGIN
  -- Obtener porcentaje de ganancia general y porcentaje para inversores
  SELECT COALESCE(valor::NUMERIC, 5) INTO v_porcentaje_ganancia
  FROM configuracion_sistema WHERE clave = 'porcentaje_ganancia_general';
  ...
  LOOP
    -- Calcular ganancia propia del partner (siempre 5% de su inversión)
    ...
    IF v_partner.tipo = 'operador_partner' THEN
      -- Ganancia operador: 100% de su propia ganancia
      v_ganancia_operador := v_ganancia_propia;
      
      -- Ganancia por comisión: porcentaje_comision% del 30% de ganancias de sus inversores
      FOR v_inversores IN
        SELECT i.total
      ...
      LOOP
        ...
        BEGIN
          ...
          v_parte_partners := (v_ganancia_inversor * (100 - v_porcentaje_inversores)) / 100; -- 30% por defecto
    ...
    -- Si es solo Partner
    ELSE
      -- Ganancia propia: 70% de su ganancia (como inversor normal)
      ...
      LOOP
        ...
        BEGIN
          ...
          -- Partner normal se lleva porcentaje_comision% del 30%
          v_comision_partner := (v_parte_partners * v_partner.porcentaje_comision) / 100;
...
BEGIN
  ...
  SELECT SUM(total) INTO v_total_inversion FROM inversores;
...
-- Función para procesar ganancias semanales con cálculo correcto
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      ELSE
        -- Partner normal: 70% de su ganancia + porcentaje_comision% del 30% de sus inversores
        v_ganancia_operador := (v_ganancia_propia * v_porcentaje_inversores) / 100;
