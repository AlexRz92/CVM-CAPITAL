  ...
  SELECT 
    ...
    CASE 
      WHEN COUNT(pi.inversor_id) > 0 THEN 
        (COALESCE(SUM(i.total), 0) / calcular_total_inversion()) * p_ganancia_partners * (p.porcentaje_comision / 100)
      ELSE 0 
    END as ganancia_comision,
    CASE 
      WHEN p.tipo = 'operador_partner' AND COUNT(pi.inversor_id) > 0 THEN 
        (COALESCE(SUM(i.total), 0) / calcular_total_inversion()) * p_ganancia_partners * (p.porcentaje_especial / 100)
      ELSE 0 
