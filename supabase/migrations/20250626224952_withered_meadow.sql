/*
  # Reestructuración completa de la base de datos CVM Capital
  
  1. Eliminar campos de porcentajes personalizables de partners
  2. Establecer valores predefinidos para partners y operadores
  3. Limitar campos numéricos a máximo 2 decimales
  4. Mejorar funciones para nuevos requerimientos
  5. Añadir funciones para gráficos de torta y listados mejorados
*/
...
-- Tabla configuracion_sistema (para valores numéricos)
-- Nota: valor es text, pero se puede validar en la aplicación
...
-- =============================================
-- PASO 4: ACTUALIZAR FUNCIÓN DE PROCESAMIENTO DE GANANCIAS
-- =============================================
...
-- =============================================
-- PASO 5: FUNCIÓN PARA GRÁFICO DE TORTA DE PARTNERS
-- =============================================
...
BEGIN
  ...
  SELECT inversion_inicial INTO v_inversion_inicial
  FROM partners 
  WHERE id = p_partner_id;

  -- Calcular depósitos (incluyendo inversión inicial)
  SELECT COALESCE(SUM(monto), 0) INTO v_depositos
  FROM partner_transacciones 
  WHERE partner_id = p_partner_id 
    AND tipo IN ('deposito', 'depósito');
  
  -- Agregar inversión inicial a depósitos
  v_depositos := v_depositos + COALESCE(v_inversion_inicial, 0);
...
-- =============================================
-- PASO 6: FUNCIÓN PARA LISTADO DE INVERSORES CON GANANCIAS
-- =============================================
...
-- =============================================
-- PASO 7: FUNCIÓN PARA VALIDAR ELIMINACIÓN DE PARTNER
-- =============================================
...
SELECT 'Función obtener_distribucion_partners actualizada' as status
WHERE EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'obtener_distribucion_partners');

SELECT 'Función procesar_ganancias_semanales actualizada' as status
WHERE EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'procesar_ganancias_semanales');

SELECT 'Función obtener_datos_torta_partner creada' as status
WHERE EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'obtener_datos_torta_partner');

SELECT 'Función obtener_inversores_con_ganancias_partner creada' as status
WHERE EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'obtener_inversores_con_ganancias_partner');

SELECT 'Función validar_eliminacion_partner creada' as status
WHERE EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'validar_eliminacion_partner');

-- Mostrar estructura actualizada de partners
SELECT 
  column_name, 
  data_type, 
  numeric_precision, 
...
/*
  ...
  5. **Mejoras en funciones**:
     - Cálculos redondeados a 2 decimales
     - Mejor separación de tipos de ganancias
